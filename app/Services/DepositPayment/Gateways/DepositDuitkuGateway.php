...
/**
 * Integrasi Duitku API v2 untuk topup saldo. Logikanya sama persis dengan
 * App\Services\Payment\Gateways\DuitkuGateway (form_payments), cuma
 ...
 */
class DepositDuitkuGateway implements DepositPaymentGatewayInterface
{
    ...
    public function createTransaction(DepositPayment $payment, ?string $paymentMethod = null): array
    {
        ...
        $response = Http::acceptJson()->post($this->inquiryUrl(), [
            ...
            'callbackUrl' => route('deposit.webhook.duitku'),
            'signature' => $signature,
            'expiryPeriod' => (int) ($this->config->expiry_minutes ?? 60),
        ]);

        if ($response->failed()) {
            Log::error('[DEPOSIT][Duitku] Gagal membuat transaksi', [
                'order_id' => $payment->order_id,
                'status' => $response->status(),
                'body' => $response->json(),
            ]);
