...
/**
 * Integrasi Midtrans Snap untuk topup saldo. Logikanya sama persis dengan
 * App\Services\Payment\Gateways\MidtransGateway (form_payments), cuma
 * dioperasikan di atas DepositPayment supaya tidak menyentuh kode form
 * payment yang sudah live -- lihat docblock
 * DepositPaymentGatewayInterface.
 *
 * Referensi resmi: https://docs.midtrans.com/docs/snap-snap-integration-guide
 */
class DepositMidtransGateway implements DepositPaymentGatewayInterface
{
    ...
    public function createTransaction(DepositPayment $payment, ?string $paymentMethod = null): array
    {
        ...
        if ($response->failed()) {
            Log::error('[DEPOSIT][Midtrans] Gagal membuat transaksi', [
                'order_id' => $payment->order_id,
