...
/**
 * Integrasi Duitku API v2 untuk checkout package. Logikanya sama persis
 * dengan App\Services\DepositPayment\Gateways\DepositDuitkuGateway, cuma
 ...
 */
class CoursePackageDuitkuGateway implements CoursePackagePaymentGatewayInterface
{
    ...
    public function createTransaction(CoursePackagePayment $payment, ?string $paymentMethod = null): array
    {
        ...
        $response = Http::acceptJson()->post($this->inquiryUrl(), [
            ...
            'productDetails' => 'Pembelian Package (sisa setelah saldo)',
            'email' => $payment->email,
            'paymentMethod' => $paymentMethod,
            'customerVaName' => $payment->name,
            'returnUrl' => route('inayule.checkout.return', ['order_id' => $payment->order_id]),
            'callbackUrl' => route('course-package.webhook.duitku'),
            'signature' => $signature,
            'expiryPeriod' => (int) ($this->config->expiry_minutes ?? 60),
        ]);

        if ($response->failed()) {
            Log::error('[COURSE-PACKAGE][Duitku] Gagal membuat transaksi', [
                'order_id' => $payment->order_id,
                'status' => $response->status(),
                'body' => $response->json(),
            ]);
