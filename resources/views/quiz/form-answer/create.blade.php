...
<div class="middle-content container-xxl p-0">
    ...
    <form action="{{ route('quiz.form-answer.store') }}" method="POST">
        @csrf
                            ...
                            <select class="form-select @error('submission_id') is-invalid @enderror" id="submission_id"
                                ...
                                @foreach ($submissions as $submission)
                                    <option value="{{ $submission->id }}"
                                        {{ old('submission_id') == $submission->id ? 'selected' : '' }}>
                                        {{ $submission->id }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-4">
                        <div class="col-sm-12">
                            ...
                            <textarea class="form-control @error('answer_text') is-invalid @enderror" id="answer_text" name="answer_text"
                                rows="4" placeholder="Enter answer text...">{{ old('answer_text') }}</textarea>
                            @error('answer_text')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="widget-content widget-content-area ecommerce-create-section">
                            <div class="row">
                                <div class="col-sm-12 mb-3">
                                    <button type="submit" class="btn btn-success w-100">Create Answer</button>
                                </div>
                                <div class="col-sm-12">
