...
<div class="middle-content container-xxl p-0">
    ...
    <form action="{{ route('pembayaran.form.store') }}" method="POST">
        @csrf
                    ...
                    <div class="row mb-4">
                            ...
                            <select class="form-select @error('pembayaran_category_id') is-invalid @enderror"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('pembayaran_category_id') === $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-6">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name') }}" placeholder="Enter form name...">
                            @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-sm-6">
                            ...
                            <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount"
                                name="amount" value="{{ old('amount') }}" min="0" placeholder="Enter amount...">
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
