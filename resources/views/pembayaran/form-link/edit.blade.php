...
<div class="middle-content container-xxl p-0">
    ...
    <form action="{{ route('pembayaran.form-link.update', $data->id) }}" method="POST">
        @csrf
        @method('PUT')
                            ...
                            <select class="form-select @error('parent_id') is-invalid @enderror" id="parent_id" name="parent_id">
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ old('parent_id', $data->parent_id) === $user->id ? 'selected' : '' }}>
                            ...
                            <select class="form-select @error('pembayaran_form_id') is-invalid @enderror"
                                ...
                                @foreach ($forms as $form)
                                    <option value="{{ $form->id }}"
                                        {{ old('pembayaran_form_id', $data->pembayaran_form_id) === $form->id ? 'selected' : '' }}>
                                        {{ $form->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-6">
                            ...
                            <select class="form-select @error('payment_status') is-invalid @enderror" id="payment_status"
                                ...
                                <option value="pending"
                                    {{ old('payment_status', $data->payment_status) === 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="paid"
                                    {{ old('payment_status', $data->payment_status) === 'paid' ? 'selected' : '' }}>Paid</option>
                                <option value="failed"
                                    {{ old('payment_status', $data->payment_status) === 'failed' ? 'selected' : '' }}>Failed</option>
                                <option value="cancelled"
                                    {{ old('payment_status', $data->payment_status) === 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                            </select>
                    ...
                    <div class="row mb-4">
                        <div class="col-sm-6">
                            ...
                            <input type="date" class="form-control @error('payment_date') is-invalid @enderror"
                                id="payment_date" name="payment_date" value="{{ old('payment_date', $data->payment_date) }}">
                            @error('payment_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-sm-6">
                            ...
                            <input type="text" class="form-control @error('order_id') is-invalid @enderror"
                                id="order_id" name="order_id" value="{{ old('order_id', $data->order_id) }}"
                        ...
                        <div class="widget-content widget-content-area ecommerce-create-section">
                            <div class="row">
                                <div class="col-sm-12 mb-3">
                                    <button type="submit" class="btn btn-success w-100">Update Form Link</button>
                                </div>
                                <div class="col-sm-12">
