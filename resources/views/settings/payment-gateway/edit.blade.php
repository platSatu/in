...
<div class="middle-content container-xxl p-0">
    ...
    <form action="{{ route('settings.payment-gateway.update', $data->id) }}" method="POST">
        @csrf
        @method('PUT')
                    ...
                    <div class="row mb-4">
                        <div class="col-sm-6">
                            ...
                            <select class="form-select @error('gateway') is-invalid @enderror" id="gateway" name="gateway">
                                ...
                                <option value="duitku" {{ old('gateway', $data->gateway) === 'duitku' ? 'selected' : '' }}>Duitku</option>
                                <option value="midtrans" {{ old('gateway', $data->gateway) === 'midtrans' ? 'selected' : '' }}>Midtrans</option>
                                <option value="ipaymu" {{ old('gateway', $data->gateway) === 'ipaymu' ? 'selected' : '' }}>iPaymu</option>
                            </select>
                            @error('gateway')
                        ...
                        <div class="col-sm-6">
                            ...
                            <select class="form-select @error('environment') is-invalid @enderror" id="environment" name="environment">
                                <option value="sandbox" {{ old('environment', $data->environment) === 'sandbox' ? 'selected' : '' }}>Sandbox (testing)</option>
                                <option value="production" {{ old('environment', $data->environment) === 'production' ? 'selected' : '' }}>Production (live)</option>
                            </select>
                                    ...
                                    @foreach ($fields as $fieldKey => $label)
                                        <div class="col-sm-6">
                                            ...
                                            <input type="text"
                                                ...
                                                data-field="{{ $fieldKey }}"
                                                placeholder="{{ $label }}"
                                                value="{{ old('credentials.' . $fieldKey, $data->gateway === $gatewayKey ? ($data->credentials[$fieldKey] ?? '') : '') }}">
                            ...
                            <div class="row">
                                <div class="col-xxl-12 mb-4">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" role="switch" id="is_active"
                                            name="is_active" value="1" {{ old('is_active', $data->is_active) ? 'checked' : '' }}>
                                ...
                                <div class="col-xxl-12 mb-4">
                                    ...
                                    <input type="number" min="5" max="1440"
                                        ...
                                        value="{{ old('expiry_minutes', $data->expiry_minutes ?? 60) }}">
