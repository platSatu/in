...
<div class="middle-content container-xxl p-0">
    ...
    <div class="widget-content widget-content-area layout-top-spacing">
        ...
        <form action="{{ route('company.branch.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <select class="form-select @error('company_profile_id') is-invalid @enderror"
                        ...
                        @foreach ($companyProfiles as $companyProfile)
                            <option value="{{ $companyProfile->id }}"
                                {{ old('company_profile_id', $data->company_profile_id) == $companyProfile->id ? 'selected' : '' }}>
                                {{ $companyProfile->name }}
                            </option>
                        @endforeach
                    </select>
